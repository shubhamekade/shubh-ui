...
const { ChevronUpIcon, ChevronDownIcon, MagnifyingGlassIcon, ChevronLeftIcon, ChevronRightIcon } = HeroIcons as Record<string, any>;
...
const ServerDataTable = <T extends Record<string, unknown>>({
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg mb-2" style={{ background: tk.headerBg }}>
        {columns.map((col) => (
          <div
            key={col.key}
            style={colStyle}
          ...
          >
            ...
            {col.sortable && <SortIcon direction={sortField === col.key ? sortDirection : null} activeClass={tk.sortIconActive} />}
          </div>
        ))}
      ...
      <div>
        {loading ? (
          Array.from({ length: pageSize > 5 ? 5 : pageSize }).map((_, i) => (
            <div key={i}>
              <SkeletonRow cols={colCount} tk={tk} />
            </div>
          ))
        ) : data.length === 0 ? (
            ...
            return (
              <div key={key} className="mb-2">
                ...
                >
                  ...
                  >
                    {isExpanded ? <ChevronUpIcon className={`h-4 w-4 ${tk.expandBtnIcon}`} /> : <ChevronDownIcon className={`h-4 w-4 ${tk.expandBtnIcon}`} />}
                  </button>
                </div>
