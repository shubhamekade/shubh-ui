...
const faqItems = [
  {
    ...
    content: 'Run npm install @shubh/ui in your project. All components are tree-shakeable — only the components you import will be included in your bundle.',
  },
  {
    id: 'q2',
    title: 'Can I use custom Tailwind classes alongside the component props?',
    content: 'Yes. Every component accepts a className prop that is merged using tailwind-merge, so your custom classes safely override defaults without conflicts.',
  },
  {
    id: 'q3',
    title: 'Are all components accessible?',
    content: 'Yes. All interactive components implement ARIA roles, keyboard navigation, focus management, and screen reader labels per WCAG 2.1 AA guidelines.',
  },
  {
    id: 'q4',
    title: 'How are variants managed?',
    content: 'Variants use class-variance-authority (CVA). Each component exports a variants object you can import to compose custom variant logic.',
  },
  {
    id: 'q5',
    title: 'Is dark mode supported?',
    content: 'Dark mode support is on the roadmap for v2.0. Currently all components are optimized for the light theme with the Navy brand palette.',
    icon: <Badge variant="warning" size="sm">Planned</Badge>,
  },
];

const multipleItems = [
  { id: 'layout',   title: 'Layout components',   icon: <Package className="h-4 w-4 text-[#000080]" />, content: 'Container, Grid, Stack, Divider — structural layout primitives.' },
  { id: 'form',     title: 'Form components',     icon: <Package className="h-4 w-4 text-green-600" />, content: 'Button, Input, Textarea, Select, Checkbox, Radio, Switch, Label, FormField, FormError.' },
  { id: 'feedback', title: 'Feedback components', icon: <Package className="h-4 w-4 text-amber-500" />, content: 'Alert, Toast, Progress, Spinner, Skeleton — state and notification indicators.' },
];
...
;`}
      ...
      <ShowcaseSection
        ...
        props={[
          ...
          { name: 'variant', type: "'default' | 'bordered' | 'flush' | 'filled'", default: "'default'", description: 'Visual style variant' },
        ]}
      >
