...
import { HashtagIcon as Hash, BoltIcon as Zap, CodeBracketIcon as Code2 } from '@heroicons/react/24/outline';
...
;`}
      ...
      <ShowcaseSection
        ...
        props={[
          { name: 'color', type: "'default' | 'blue' | 'green' | 'red' | 'amber' | 'purple' | 'pink'", default: "'default'", description: 'Tag color scheme' },
      ...
      >
        <div className="w-full space-y-4">
          ...
          <div className="flex items-center justify-center gap-3 p-6 bg-gray-50 rounded-lg border border-[#d7d7d7] flex-wrap">
            ...
            <Tag color={ctrl.variant as any || 'blue'} size={ctrl.size as any || 'md'} icon={<Hash className="h-3 w-3" />}>
              With icon
            </Tag>
            <Tag color={ctrl.variant as any || 'blue'} size={ctrl.size as any || 'md'} removable>
              Removable
            </Tag>
          </div>
        </div>
...
<Tag color="pink">Pink</Tag>`}
      ...
      >
        <Tag color="blue"   icon={<Hash className="h-3 w-3" />}>Next.js</Tag>
        <Tag color="green"  icon={<Code2 className="h-3 w-3" />}>TypeScript</Tag>
        <Tag color="amber"  icon={<Zap className="h-3 w-3" />}>Tailwind</Tag>
        <Tag color="purple" icon={<Hash className="h-3 w-3" />}>React</Tag>
      </ShowcaseSection>
      ...
      >
        <div className="w-full max-w-sm space-y-3">
          <div className="flex flex-wrap gap-2">
            ...
            {tags.length === 0 && (
              <span className="text-sm text-[#808080]">All tags removed.</span>
            )}
          </div>
