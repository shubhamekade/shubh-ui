...
const ALL_ROWS: RequestRow[] = [
  ...
  },
  {
    ...
    requestType: 'Gift voucher',
    amount: 5000,
    appliedOn: '30 Dec, 2024',
    status: 'active',
  },
  {
    id: '3',
  ...
  },
  {
    ...
    name: 'Employee 4',
    department: 'Department 2',
    requestType: 'Gift voucherce',
    amount: 1500,
    appliedOn: '14 Jan, 2025',
    status: 'active',
  },
  {
    id: '5',
...
/>`}
        props={[
          {
            name: 'fetchData',
            type: '(params: FetchParams) => Promise<FetchResult<T>>',
            required: true,
            description: 'Async function called on every page/search/sort/status change',
          },
          {
            name: 'columns',
            type: 'ColumnDef<T>[]',
            required: true,
            description:
              'Column definitions with key, header, sortable flag, and optional cell renderer',
          },
          {
            name: 'keyExtractor',
            type: '(row: T) => string',
            required: true,
            description: 'Unique key per row for React reconciliation',
          },
          {
            name: 'pageSize',
            type: 'number',
          ...
          {
            name: 'appliedDateField',
            type: 'keyof T',
            description: 'Field name used to display "Applied on" date in expanded row',
          },
          {
            name: 'onApprove',
            type: '(row: T, note: string) => void',
            description: 'Called when Approve button is clicked in expanded row',
          },
          {
            name: 'onReject',
            type: '(row: T, note: string) => void',
            description: 'Called when Reject button is clicked in expanded row',
          },
          {
            name: 'renderExpandedRow',
            type: '(row, note, onNoteChange, onApprove, onReject) => ReactNode',
            description: 'Custom expanded row renderer — overrides default Note/Approve/Reject UI',
          },
          {
            name: 'theme',
            type: "'navy' | 'light'",
            default: "'light'",
            description: 'Color theme — navy dark or light',
          },
        ]}
      >
